	...
	*/
	int _get_MACTest(struct addr_tuple *myAddr, int numTierAddr) {
		...
		char labelAssignmentPayLoad[200];
			...
			//printf("myTierValue : %d",myTierValue);
			//printf("tierValue : %d",tierValue);
				...
				if (ethhead != NULL) {
					...
					if (checkMSGType == 1 && strcmp(recvOnEtherPort, "eth0") != 0) {
						...
						for (; z < tierAddrTotal; z++) {
							...
							//printf("VAR : %d : %d",stablePort,programStableFlag);
						...
						//	printf("\n Hello message from - recvOnEtherPort=%s \n",recvOnEtherPort); //uncomment it later
								...
								// if (z == 0) { //12/1/20121 - added programStableFlag to avoid garbage value --  && programStableFlag==0
					...
					if (checkMSGType == 5 && strcmp(recvOnEtherPort, "eth0") != 0) {
						...
						if(myTierValue==1){
							...
							//printf("\nReceived ip address = %s and its  and need to send it my parent %s via %s",inet_ntoa(ipAddr),parentTierAdd,fwdInterface);
					...
					if (checkMSGType == MESSAGE_TYPE_JOIN) {
						...
						if (myTierValue < tierValueRequestedNode && strcmp(recvOnEtherPort,"eth0")!=0) {
							...
							//if(findStable(recvOnEtherPort)==0){
					...
					if (checkMSGType == 13 && strcmp(recvOnEtherPort, "eth0") != 0) {
						...
						if (chkDestLbl==0) 
						{
							...
							uint8_t ipLen = ethhead[lengthIndex];
								...
								freeInterfaces();
								interfaceListSize = 0;
								// printf("\nipAddr: %s", inet_ntoa(ipAddr));
								// responseIPresolve(tierAddrTemp, ipAddr, MytierAddrTemp,totalEntries,0);
						}
